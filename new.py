    ...
    if uploaded_ids is not None and uploaded_mapping is not None:
        ...
        try:
            if uploaded_mapping.name.endswith(('.xls', '.xlsx')):
                # Используем converters для корректного чтения ID без десятичной части
                df_mapping = pd.read_excel(uploaded_mapping, converters={0: lambda x: str(int(x)) if not pd.isnull(x) else ''})
            else:
                df_mapping = pd.read_csv(uploaded_mapping, dtype={df_mapping.columns[0]: str})
